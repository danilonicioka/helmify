...
import (
	...
	"github.com/danilonicioka/helmify/pkg/cluster"
...
func (c cert) Process(appMeta helmify.AppMetadata, obj *unstructured.Unstructured) (bool, helmify.Template, error) {
	...
	for _, dnsName := range dnsNames {
		...
		processedDns := templatedDns
		if appMeta.Namespace() != "" {
			processedDns = strings.ReplaceAll(processedDns, appMeta.Namespace(), "{{ .Release.Namespace }}")
		}
