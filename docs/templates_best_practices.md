...
It is highly recommended that new charts are created via the `helm create` command as the template names are automatically defined as per this best practice.
...
For example, if a required function is introduced to the above example, and `maxMem` is unset, then a `#` YAML comment will introduce a rendering error.
...
**Incorrect:** helm template returns error
```gotemplate
{{- /*
# This may cause problems if the value is more than 100Gi
# memory: {{ required "maxMem must be set" .Values.maxMem | quote }}
